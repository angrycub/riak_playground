...
if [ ! -f "/vagrant/data/rpmcache/riak-cs-1.4.5-1.el6.x86_64.rpm" ] 
  then
    echo "   - Downloading Riak CS 1.4.5 Package into cache"
    wget -q --output-document=/vagrant/data/rpmcache/riak-cs-1.4.5-1.el6.x86_64.rpm http://s3.amazonaws.com/downloads.basho.com/riak-cs/1.4/1.4.5/rhel/6/riak-cs-1.4.5-1.el6.x86_64.rpm
fi

echo "* Installing Riak CS Package"
yum -y --nogpgcheck --noplugins localinstall /vagrant/data/rpmcache/riak-cs-1.4.5-1.el6.x86_64.rpm
...
cat xx00 /vagrant/data/templates/riak-cs_1.4.5_riak_config.fragment.1 xx01 > app.config
rm -f xx* /etc/riak/app.config.tmp

mv /etc/riak/app.config /etc/riak/app.config.tmp
csplit /etc/riak/app.config.tmp "/{riak_core,/+1"
cat xx00 /vagrant/data/templates/riak-cs_1.5.2_riak_config.fragment.2 xx01 > app.config
rm -f xx* /etc/riak/app.config.tmp
