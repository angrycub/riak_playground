...
JSONFile="/vagrant/.cluster/cluster.json"
...
insert_service() {
  if [ -z "$2" ]
    then
      echo "insert_service requires 2 parameters -- insert_service »service_name« »identifier«"
      exit 1
  fi
...
get_service() {
  if [ -z "$1" ]
    then
      echo "get_service requires 1 parameter -- get_service »key«"
      exit 1
  fi  
